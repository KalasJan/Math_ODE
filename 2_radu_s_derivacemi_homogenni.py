...
# podminky
podminky = [
    sym.Eq(sym.diff(vysledek, x).subs(x, 0), 0),  # y'(0) = 0
    sym.Eq(sym.diff(vysledek, x).subs(x, 1), 2)] # y'(1) = 2
...
hx = num.linspace(-0.5, 2, 1000)
hy = f(hx)

# samotne vykresleni:
mal.figure(figsize=(10, 6))
mal.plot(hx, hy, label=f"$y = {substituce}$", color="blue")
mal.axhline(0, color='black', linewidth=0.5, linestyle='--')
mal.axvline(0, color='black', linewidth=0.5, linestyle='--')
mal.title("Řešení diferenciální rovnice $y'' + 2y' -y +1 = 0$")
mal.xlabel("x")
mal.ylabel("y")
mal.ylim(4,8) # ohraniceni osy y
mal.grid(True)
